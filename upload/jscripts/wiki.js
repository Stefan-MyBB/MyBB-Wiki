...
Wiki.prototype = {
	...
	initialize: function(textarea, options)
	{
		...
		$$('script').each(function(script) {
			if(script.src && script.src.indexOf('wiki.js') != -1) {
				this.baseURL = script.src.replace(/wiki\.js(.*?)$/, '');
			}
		}, this);
	...
	{		
		...
		this.createToolbar('formatting', {
			...
			items: [
				...
				{type: 'separator'},
				{type: 'button', name: 'h1', insert: 'h1', image: 'h1.gif', title: this.options.lang.title_h1},
				{type: 'button', name: 'h2', insert: 'h2', image: 'h2.gif', title: this.options.lang.title_h2},
				{type: 'button', name: 'wiki', insert: 'wiki', image: 'wiki.gif', title: this.options.lang.title_wiki},
				{type: 'button', name: 'toc', insert: 'toc', image: 'toc.gif', title: this.options.lang.title_toc},
				{type: 'separator'}
			]
		});
	...
	insertwiki: function()
	{
		if(this.getSelectedText($(this.textarea)))
		{
			add = '?'+encodeURIComponent(this.getSelectedText($(this.textarea)));
		}
		else
		{
			add = '';
		}
		MyBB.popupWindow(popup_path+add, 'wiki', 600, 600);
	},

	inserttoc: function()
	{
		this.performInsert("[toc]", "", true, false);
	},
	...
	insertFile: function(aid)
	{
		this.performInsert("[f="+aid+"]", "", true);
	},

	insertImage: function(aid)
	{
		text = prompt(this.options.lang.enter_description, '');

		if(text)
		{
			this.performInsert("[f="+aid+"]"+text+"[/f]", "", true);
		}
	}
};
